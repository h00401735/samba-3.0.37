		 ...
		 DOM_SID *ret_sid, enum lsa_SidType *ret_type)
{
	...
	if (p != NULL) {
		domain = talloc_strndup(tmp_ctx, full_name, PTR_DIFF(p, full_name));
	...
	DEBUG(10,("lookup_name: %s => %s (domain), %s (name), flags = 0x0%x\n",
		full_name, domain, name, flags));
	...
	if ((winbind_lookup_name(lp_workgroup(), name, &sid, &type))) {
	...
	if (IS_DC && winbind_lookup_name("", name, &sid, &type)) {
		...
		if (!winbind_lookup_sid(tmp_ctx, &dom_sid, &domain, NULL, &domain_type) ||
