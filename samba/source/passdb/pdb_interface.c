...
static struct pdb_methods *pdb_get_methods_reload( BOOL reload )
{
	...
	if ( pdb && reload ) {
		...
		if ( !NT_STATUS_IS_OK(make_pdb_method_name( &pdb, SMB_PASSDB_BACKEND) ) ) {
			smb_panic("pdb_get_methods_reload: failed to get pdb methods\n");
		}
	}

	if ( !pdb ) {
		if ( !NT_STATUS_IS_OK(make_pdb_method_name( &pdb, SMB_PASSDB_BACKEND))) {
			smb_panic("pdb_get_methods_reload: failed to get pdb methods for backend");
		}
	}
				  ...
				  union unid_t *id, enum lsa_SidType *type)
{
	...
	if (sid_peek_check_rid(&global_sid_Builtin, sid, &rid)) {
		...
		if ((map.sid_name_use != SID_NAME_ALIAS) && (map.sid_name_use != SID_NAME_WKN_GRP)) {
			DEBUG(10, ("Map for sid %s is a %s, expected an alias\n",
				sid_string_static(sid), sid_type_lookup(map.sid_name_use)));
			goto done;
		}
...
struct pdb_search *pdb_search_groups(void)
{
	...
	if (pdb == NULL || pdb->search_groups == NULL) {
		return NULL;
	}
...
struct pdb_search *pdb_search_aliases(const DOM_SID *sid)
{
	...
	if (pdb == NULL || pdb->search_aliases == NULL) {
		return NULL;
	}
