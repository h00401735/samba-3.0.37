...
static struct smb_passwd *getsmbfilepwent(struct smbpasswd_privates *smbpasswd_state, FILE *fp)
{
	...
	while (status && !feof(fp)) {
		...
		if (strnequal((char *) p, LOCAL_SMB_NO_PASSWORD, strlen(LOCAL_SMB_NO_PASSWORD))) {
...
static BOOL del_smbfilepwd_entry(struct smbpasswd_privates *smbpasswd_state, const char *name)
{
	 ...
	 * Scan the file, a line at a time and check if the name matches.
	 */
	DEBUG(10, ("begin search for user [%s].\n", name));
	while ((pwd = getsmbfilepwent(smbpasswd_state, fp)) != NULL) {
		char *new_entry;
		size_t new_entry_length;
				  ...
				  struct samu *sam_acct, const char *username)
{
	...
	DEBUG(10, ("getsampwnam (smbpasswd): search by name: %s(file:%s)\n", username, smbpasswd_state->smbpasswd_file));
...
static inline BOOL smbpasswd_rid_algorithm(struct pdb_methods *methods)
{
	return True;
}
...
static NTSTATUS pdb_init_smbpasswd( struct pdb_methods **pdb_method, const char *location )
{
	...
	DEBUG(5, ("Init SMBPASSWD.\n"));

	(*pdb_method)->name = SMB_PASSDB_BACKEND;
	...
	if (location) {
		...
		privates->smbpasswd_file = talloc_strdup(*pdb_method, SMB_PASSWD_FILE);
	}
...
{
	return smb_register_passdb(PASSDB_INTERFACE_VERSION, SMB_PASSDB_BACKEND, pdb_init_smbpasswd);
}
