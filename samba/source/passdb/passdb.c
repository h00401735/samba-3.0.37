			   ...
			   char *msg_str, size_t msg_str_len)
{
	...
	*msg_str = '\0';
	...
	become_root();
	BOOL found = pdb_getsampwnam(sam_pass, user_name);
	unbecome_root();
	if(!found) {
		TALLOC_FREE(sam_pass);

		if ((local_flags & LOCAL_DELETE_USER) != 0)
		{
			slprintf(msg_str, msg_str_len-1, "Deleted user %s(user do not exist).\n", user_name);
			return NT_STATUS_OK;
		}
		if(local_flags & LOCAL_ADD_USER) {
			...
			if ( !(pwd = getpwnam_alloc( NULL, user_name)) ) {
				slprintf(err_str, err_str_len-1,"Failed to find entry in system.\n");
				return NT_STATUS_NO_SUCH_USER;
			}
			...
			if ( !(sam_pass = samu_new( NULL )) ) {
				slprintf(err_str, err_str_len-1,"Failed to alloc user struct.\n");
				return NT_STATUS_NO_MEMORY;
			}
			...
			if (NT_STATUS_EQUAL(result, NT_STATUS_INVALID_PRIMARY_GROUP)) {
				slprintf(err_str, err_str_len-1,"wocao ...\n");
				return result;
			}
	...
	{
		/* HM3, always do this, set to normal. */
