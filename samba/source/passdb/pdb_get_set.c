...
BOOL pdb_set_nt_passwd(struct samu *sampass, const uint8 pwd[NT_HASH_LEN], enum pdb_value_state flag)
{
	...
	if (pwd) {
		sampass->nt_pw = data_blob_talloc(sampass, pwd, NT_HASH_LEN);
	} else {
...
{
	if (historyLen && pwd){
		sampass->nt_pw_his = data_blob_talloc(sampass, pwd, historyLen*PW_HISTORY_ENTRY_LEN);
...
{
	if (sampass->backend_private_data && sampass->backend_private_data_free_fn) {
		sampass->backend_private_data_free_fn(&sampass->backend_private_data);
	}
