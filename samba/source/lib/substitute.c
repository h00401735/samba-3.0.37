...
void set_remote_machine_name(const char* remote_name, BOOL perm)
{
	...
	if (already_perm) // HM3: permanent
		return;

	already_perm = perm;

	fstrcpy(tmp, remote_name);
	trim_char(tmp, ' ', ' ');
	alpha_strcpy(remote_machine, tmp, SAFE_NETBIOS_CHARS, sizeof(remote_machine)-1);
...
char *alloc_sub_basic(const char *smb_name, const char *domain_name, const char *str)
{
	...
	for (b = s = a_string; (p = strchr_m(s, '%')); s = a_string + (p - b)) {
		...
		case 'd' :
			...
			break;
		case 'h' :
			a_string = realloc_string_sub(a_string, "%h", global_myname());
			break;
		case 'm' :
