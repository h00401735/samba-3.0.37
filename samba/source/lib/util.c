...
********************************************************************/
// HM3: shit, why not use dup2 instead?
void close_low_fds(BOOL stderr_too)
{
	...
	if (fd < 0) {
		DEBUG(0,("Can't open /dev/null, errno %d\n", errno));
		return;
	}
	dup2(fd, 0);
	dup2(fd, 1);
	if (stderr_too) dup2(fd, 2);
	close(fd);
#endif
}
...
int set_blocking(int fd, BOOL set)
{
	...
	return old_val != val ? sys_fcntl_long( fd, F_SETFL, val) : 0;
}
...
char *lock_path(const char *name)
{
	...
	fstr_sprintf(fname, "%s/%s", LOCKDIR, name);
	return fname;
}
...
{
	static pstring fname;
	fstr_sprintf(fname, "%s/%s", PIDDIR, name);
	return fname;
}
...
{
	static pstring fname;
	fstr_sprintf(fname, "%s/%s", LIBDIR, name);
	return fname;
}
