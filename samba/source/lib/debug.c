...
BOOL reopen_logs( void )
{
	...
	// HM3: here comes two case: 1.log-file override in configuration. 2.only print after loaded.
	if (lp_loaded()) {
		const char *logfname = lp_logfile();
		if (*logfname)
	...
	if (dbf && sys_dup2(x_fileno(dbf), 2) == -1) {
		close_low_fds(True); /* Close stderr too, if dup2 can't point it at the logfile */
	}
...
BOOL dbghdr( int level, const char *file, const char *func, int line )
{
	...
	if (True) {
		...
		if( lp_debug_prefix_timestamp() ) {
			(void)Debug1( "[%s, %d%s] ",
				current_timestring(lp_debug_hires_timestamp()), level, header_str);
		} else {
