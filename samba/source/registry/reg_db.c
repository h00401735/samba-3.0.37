...
static BOOL init_registry_data( void )
{
	...
	for ( i=0; builtin_registry_paths[i] != NULL; i++ ) {
		...
		DEBUG(11,("init_registry_data: Adding [%s]\n", builtin_registry_paths[i]));
		...
		while ( next_token(&p, keyname, "\\", sizeof(keyname)) ) {
			...
			*subkeyname = '\0';
			...
			DEBUG(11,("init_registry_data: Storing key [%s] with subkey [%s]\n",
...
int regdb_fetch_values( const char* key, REGVAL_CTR *values )
{
	...
	DEBUG(11,("regdb_fetch_values: Looking for value of key [%s] \n", key));
...
BOOL regdb_store_values( const char *key, REGVAL_CTR *values )
{
	...
	DEBUG(11,("regdb_store_values: Looking for value of key [%s] \n", key));
