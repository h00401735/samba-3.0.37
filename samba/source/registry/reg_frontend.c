...
BOOL fetch_reg_keys_specific( REGISTRY_KEY *key, char** subkey, uint32 key_index )
{
	...
	*subkey = NULL;
	...
	DEBUG(11,("fetch_reg_keys_specific: Looking for key [%d] of  [%s]\n", key_index, key->name));

	if ( !ctr ) {
		DEBUG(11,("fetch_reg_keys_specific: Initializing cache of subkeys for [%s]\n", key->name));
	...
	/* clear the cache when key_index == 0 or the path has changed */
		...
		DEBUG(11,("fetch_reg_keys_specific: Updating cache of subkeys for [%s]\n", key->name));
...
BOOL fetch_reg_values_specific( REGISTRY_KEY *key, REGISTRY_VALUE **val, uint32 val_index )
{
	...
	*val = NULL;
	...
	if ( !ctr ) {
		DEBUG(11,("fetch_reg_values_specific: Initializing cache of values for [%s]\n", key->name));
	...
	/* clear the cache when val_index == 0 or the path has changed */
		...
		DEBUG(11,("fetch_reg_values_specific: Updating cache of values for [%s]\n", key->name));
                             ...
                             NT_USER_TOKEN *token, uint32 access_desired )
{
	...
	DEBUG(11,("regkey_open_internal: name = [%s]\n", path));
