		    ...
		    uint32_t action, uint32_t filter, const char *path)
{
 ...
 again:
	...
	for (depth=0,p=path; p && depth < notify->array->num_depths; p=next_p,depth++) {
		int p_len = p - path;
		int min_i, max_i, i;
		struct notify_depth *d = &notify->array->depth[depth];
		next_p = strchr(p+1, '/');
		...
		for (i=min_i;i<d->num_entries;i++) {
			...
			status = notify_send(notify, e,	path + e->path_len + 1, action);
			if (NT_STATUS_EQUAL(status, NT_STATUS_INVALID_HANDLE)) {
				struct server_id server = e->server;
				DEBUG(10, ("Deleting notify entries for process %s because it's gone\n",
