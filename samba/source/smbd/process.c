...
static const struct smb_message_struct smb_messages[256] =
{
...
static int switch_message(int type,char *inbuf,char *outbuf,int size,int bufsize)
{
	...
	if (smb_messages[type].fn == NULL) {
		...
		DEBUG(3,("switch message %s (flags:[0x%08x],pid:[%d]) conn 0x%lx\n",
				smb_fn_name(type), flags,(int)sys_getpid(),(unsigned long)conn));
		...
		if (conn) {
			if (!set_current_service(conn,SVAL(inbuf,smb_flg),0 != (flags & (AS_USER|DO_CHDIR)))) {
				return(ERROR_DOS(ERRSRV,ERRaccess));
			}
...
void construct_reply_common(const char *inbuf, char *outbuf)
{
	...
	SSVAL(outbuf,smb_flg2,
		(SVAL(inbuf,smb_flg2) & FLAGS2_UNICODE_STRINGS) | common_flags2);
...
#else
#define setup_select_timeout() ({blocking_locks_timeout_ms(SMBD_SELECT_TIMEOUT*1000);})
#endif

#define flushFsToMedia() do{sync();}while(0)
...
void smbd_process(void)
{
	...
	while (True) {
		...
		if (select_timeout == 0) {
			DEBUG(3,("timeout_processing: select_timeout is 0.\n"));
