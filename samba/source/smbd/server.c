...
static struct child_pid *children = NULL;
...
static BOOL open_sockets_smbd()
{
	...
	DEBUG(0, ("open_sockets_smbd: ports:[%s]!\n", SMB_PORTS));
	DEBUG(0, ("[%s/%s/%s]\n", lp_interfaces(), lp_bind_interfaces_only(), lp_socket_address()));

	if ( 0 && lp_interfaces() && lp_bind_interfaces_only()) {
		...
		*/
		...
		for(i = 0; i < num_interfaces; i++) {
			...
			for ( ptr=SMB_PORTS; next_token(&ptr, tok, " \t,", sizeof(tok)); ) {
				unsigned port = atoi(tok);
				if (port == 0 || port > 0xffff) {
					continue;
				}
				s = open_socket_in(SOCK_STREAM, port, 0, ifip->s_addr, True);
				if(s == -1)
					return False;
		...
		for ( ptr=SMB_PORTS; next_token(&ptr, tok, " \t,", sizeof(tok)); ) {
			...
			s = open_socket_in(SOCK_STREAM, port, 0, interpret_addr(lp_socket_address()),True);
			if (s == -1)
				return(False);
	...
	message_register(MSG_SMB_STAT_CACHE_DELETE, smb_stat_cache_delete, NULL);
	...
	while (1) {
		...
		DEBUG(2,("waiting for a connection, pid: %ld\n", getpid()));
		...
		for( ; num > 0; num--) {
			...
			// HM3: set to global var, each child has it's own copy.
			smbd_set_server_fd(accept(s,(struct sockaddr*)&addr,&in_addrlen));
			...
			char peer[128] = {0};
			inet_ntop(addr.sin_family, &addr.sin_addr.s_addr, peer, sizeof(peer));
			DEBUG(0,("accept client addr: %s(%d), server port %d\n", peer, ntohs(addr.sin_port), get_socket_port(smbd_server_fd())));

			/* Ensure child is set to blocking mode */
			set_blocking(smbd_server_fd(), True);

			// HM3: check if it's OK to handle this request.
			if (allowable_number_of_smbd_processes() &&
				...
				set_remote_machine_name(peer, False);

				if (!reinit_after_fork(smbd_messaging_context(), smbd_event_context(), true)) {
					DEBUG(0, ("reinit_after_fork failed.\n"));
					smb_panic("reinit_after_fork failed.\n");
				}

				// HM3: OK, I'm back.
				DEBUG(0,("child returning from smbd, pid: %ld\n", getpid()));
				return True;
			}
...
BOOL reload_services(BOOL test)
{
	...
	ret = lp_load(CONFIGFILE, False, False, True, True);
