...
BOOL vfs_init_custom(connection_struct *conn, const char *vfs_object)
{
	...
	DEBUG(3, ("Initialising custom vfs hooks from [%s] for [%s]\n", vfs_object, conn->client_address));
...
BOOL smbd_vfs_init(connection_struct *conn)
{
	...
	unsigned int i = 0;
	int j = 0;
	const char *ip = conn ? conn->client_address : "null";

	DEBUG(0, ("smbd_vfs_init for client %s\n", ip));
