...
static BOOL reload_services_file(const char *logfile)
{
	...
	if (lp_loaded()) {
		...
		if (file_exist(fname,NULL) && !strcsequal(fname, CONFIGFILE)) {
			pstrcpy(CONFIGFILE,fname);
		}
	}
...
int main(int argc, char **argv, char **envp)
{
	...
	if (!override_logfile) {
		pstr_sprintf(logfile, "%s/log.winbindd", LOGFILEBASE);
		lp_set_logfile(logfile);
	}
	...
	if (!directory_exist(LOCKDIR, NULL)) {
		mkdir(LOCKDIR, 0755);
	}
