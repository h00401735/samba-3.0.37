...
typedef struct _param_opt_struct {
	struct _param_opt_struct *prev, *next;
	char *key;
	char *value;
	char **list;
} param_opt_struct;
...
static service sDefault = {
	.valid = True,			/* valid */
	.usershare = USERSHARE_INVALID,			/* not a usershare */
	.usershare_last_mod = (time_t)0,              /* No last mod time */
	.szService = NULL,			/* szService */
	.szPath = NULL,			/* szPath */
	.szUsername = NULL,			/* szUsername */
#ifndef SMALL_BUILD
	NULL,			/* szInvalidUsers */
	NULL,			/* szValidUsers */
	NULL,			/* szAdminUsers */
...
static void init_globals(BOOL first_time_only)
{
	...
	if (!done_init) {
		...
		for (i = 0; parm_table[i].label; i++)
			if ((parm_table[i].type == P_STRING || parm_table[i].type == P_USTRING) && parm_table[i].ptr)
	...
	Globals.deadtime = 0; // HM3: default deadtime: 0.
	...
	Globals.bDebugPid = True;
	Globals.bDebugUid = True;
...
/*************Global**************/
	...
	FN_GLOBAL_STRING(lp_usershare_template, &Globals.szUsershareTemplateShare)
...
static BOOL service_ok(int iService)
{
	...
	if (!ServicePtrs[iService]->bAvailable) {
		...
		DEBUG(1, ("found Service[%d] %s, path.\n", iService,
			ServicePtrs[iService]->szService, ServicePtrs[iService]->szPath));
	}
#endif
...
{
	SMB_STRUCT_STAT lsbuf;
	const char *usersharepath = lp_usershare_path();
	pstring fname;
             ...
             BOOL initialize_globals)
{
	...
	standard_sub_basic( get_current_username(), current_user_info.domain, n2,sizeof(n2) );
	...
	if (in_client && lp_we_are_a_wins_server()) {
...
int lp_servicenumber(const char *pszServiceName)
{
	...
	DEBUG(7,("lp_servicenumber: total %d, to find %s\n", iNumServices, pszServiceName));

	for (iService = iNumServices - 1; iService >= 0; iService--) {
		DEBUG(7,("ServicePtrs[%d]: %s, valid: %d, share type: %d\n",
			iService, ServicePtrs[iService]->szService, ServicePtrs[iService]->valid, ServicePtrs[iService]->usershare));
		if (VALID(iService) && ServicePtrs[iService]->szService) {
			...
			DEBUG(7,("converted service name: %s, \n", serviceName));
	...
	if (iService >= 0 && ServicePtrs[iService]->usershare == USERSHARE_VALID) {
		...
		if (!usershare_exists(iService, &last_mod)) {
			DEBUG(0,("user share [%s] does not exists: %s is not a directory.\n", ServicePtrs[iService]->szService));
