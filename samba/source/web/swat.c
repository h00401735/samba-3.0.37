...
{
	lp_resetnumservices();
	return lp_load(CONFIGFILE,False,save_def,False,True);
}
...
static int save_reload(int snum)
{
	...
	f = sys_fopen(CONFIGFILE,"w");
	if (!f) {
		printf(_("failed to open %s for writing"), CONFIGFILE);
		printf("\n");
		return 0;
	}
...
#else
		chmod(CONFIGFILE, S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH);
#endif
	}
	...
	if (!load_config(False)) {
                printf(_("Can't reload %s"), CONFIGFILE);
		printf("\n");
                return 0;
        }
